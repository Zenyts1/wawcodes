...
class Database():
    ...
    def __getitem__(self, key):
        if key < 0:
            key += self.__len__()
        # key += 1
        return to_cli(self.cursor.execute('''
        SELECT uid, auth FROM db WHERE sid=? AND ena>0''', (key, )).fetchone())
    ...
    def __contain__(self, item):
        return self.cursor.execute('''SELECT sid FROM db WHERE uid=?''', (item["uid"],)).fetchone() is not None
    ...
    @prev_th
    def __iter__(self):
        return map(to_cli, self.cursor.execute('''SELECT uid, auth FROM db WHERE ena>0'''))

    # def __insert__(self, index, item):
    #     self.cursor.execute('''INSERT OR IGNORE INTO db (uid, auth, sid) VALUES (?,?,?)''', (item["uid"], item["auth"], self.index))
    #     self.cursor.execute('''INSERT OR IGNORE INTO db (uid, auth, sid) VALUES (?,?,?)''', (item["uid"], item["auth"], self.index))
    #     self.conn.commit()
    #     self.len_ += 1
    ...
    @prev_th
    def enumerate(self):
        return map(lambda x: (x[0], to_cli((x[1], x[2]))), self.cursor.execute('''SELECT sid, uid, auth FROM db WHERE ena>0'''))
    ...
    @prev_th
    def __repr__(self):
        return "authDatabase("+str(self.cursor.execute('''SELECT * FROM db''').fetchall())+")"
